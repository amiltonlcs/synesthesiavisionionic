...
export class BusIntegrationProvider {
	...
	getEstimacao(idEstacao: string){
		...
		return this.http.get(url, {}, {responseType: 'text'}).then((success) => {
			...
			// resultado.forEach(element => {
			// 	let data = element.arrivalTime;
			// 	var h = this.addZero(data.getUTCHours());
			// 	var m = this.addZero(data.getUTCMinutes());
			// 	var s = this.addZero(data.getUTCSeconds());

			// 	console.log("(VehicleEstimation[]) Estimativas: tempo de chegada: " + h + ":" + m + ":" + s);
			// });			
